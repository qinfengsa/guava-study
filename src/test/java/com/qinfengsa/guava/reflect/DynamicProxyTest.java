...
public class DynamicProxyTest {
    ...
    public void testJdKProxy() {
        HelloService helloService =
                new JdkInvocationHandlerImpl<HelloService>(new HelloServiceImpl()).getProxy();
    ...
    public void testGuavaDynamicProxy() {
        ...
        HelloService helloService =
                new GuavaInvocationHandlerImpl<>(HelloService.class, new HelloServiceImpl())
                        .getProxy();
        helloService.sayHello("win");
    }
     ...
     *
     * <p>一个代理实例equal另外一个代理实例，只要他们有同样的接口类型和equal的invocation handlers。
     *
     * <p>一个代理实例的toString()会被代理到invocation handler的toString()，这样更容易自定义toString()。
     *
     * <p>AbstractInvocationHandler确保传递给handleInvocation(Object, Method,
     * Object[]))的参数数组永远不会空，从而减少了空指针异常的机会。
     */
    @Test
    public void testGuavaNewDynamicProxy() {
        HelloService helloService =
                new GuavaNewInvocationHandlerImpl<>(HelloService.class, new HelloServiceImpl())
                        .getProxy();
